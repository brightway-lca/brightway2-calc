...
def empty_biosphere():
    ...
    dp = create_datapackage(
        fs=ZipFileSystem(fixture_dir / "empty_biosphere.zip", mode="w"),
    )
...
def create_basic_fixture_zipfile():
    _create_basic_fixture(ZipFileSystem(fixture_dir / "basic_fixture.zip", mode="w"))


def create_missing_characterization():
    _create_basic_fixture(
        ZipFileSystem(fixture_dir / "missing_characterization.zip", mode="w"),
...
def create_empty_characterization():
    _create_basic_fixture(
        ZipFileSystem(fixture_dir / "empty_characterization.zip", mode="w"),
...
def create_svdm_fixtures():
    dp = create_datapackage(fs=ZipFileSystem(fixture_dir / "svdm.zip", mode="w"), sequential=True)
    ...
    dp2 = create_datapackage(fs=ZipFileSystem(fixture_dir / "svdm2.zip", mode="w"))
...
def create_array_fixtures():
    ...
    # Biosphere flows: 1
    dp = create_datapackage(
        fs=ZipFileSystem(fixture_dir / "array_sequential.zip", mode="w"), sequential=True
    )
...
def create_mc_basic():
    ...
    # Activity 2
    dp = create_datapackage(
        fs=ZipFileSystem(fixture_dir / "mc_basic.zip", mode="w"),
    )
...
def create_multilca_simple():
    ...
    dp1 = create_datapackage(
        fs=ZipFileSystem(fixture_dir / "multi_lca_simple_1.zip", mode="w"),
    )
    ...
    distributions_array = np.array(
        [
            (4, 0.2, np.NaN, np.NaN, 0.1, 0.8, False),
            (4, 0.5, np.NaN, np.NaN, 0.2, 0.8, False),
            (0, 1, np.NaN, np.NaN, np.NaN, np.NaN, False),
            (0, 1, np.NaN, np.NaN, np.NaN, np.NaN, False),
            (4, 0.5, np.NaN, np.NaN, 0.2, 0.8, False),
    ...
    dp2 = create_datapackage(
        fs=ZipFileSystem(fixture_dir / "multi_lca_simple_2.zip", mode="w"),
    )
    ...
    dp3 = create_datapackage(
        fs=ZipFileSystem(fixture_dir / "multi_lca_simple_3.zip", mode="w"),
    )
    ...
    distributions_array = np.array(
        [
            (5, 0.1, np.NaN, np.NaN, 0.1, 0.8, False),
            (5, 0.2, np.NaN, np.NaN, 0.2, 0.8, False),
            (0, 1, np.NaN, np.NaN, np.NaN, np.NaN, False),
        ],
        dtype=UNCERTAINTY_DTYPE,
    )
    ...
    distributions_array = np.array(
        [
            (5, 2, np.NaN, np.NaN, 0, 10, False),
            (5, 4, np.NaN, np.NaN, 0, 10, False),
            (5, 8, np.NaN, np.NaN, 0, 10, False),
            (5, 1, np.NaN, np.NaN, 0, 10, False),
            (5, 2, np.NaN, np.NaN, 0, 10, False),
            (5, 3, np.NaN, np.NaN, 0, 10, False),
        ],
        dtype=UNCERTAINTY_DTYPE,
    )
    dp1.add_persistent_vector(
    ...
    dp4 = create_datapackage(
        fs=ZipFileSystem(fixture_dir / "multi_lca_simple_4.zip", mode="w"),
    )
    indices_array = np.array(
    ...
    dp5 = create_datapackage(
        fs=ZipFileSystem(fixture_dir / "multi_lca_simple_5.zip", mode="w"),
    )
    ...
    dp6 = create_datapackage(
        fs=ZipFileSystem(fixture_dir / "multi_lca_simple_normalization.zip", mode="w"),
    )
...
def create_mc_complete():
    ...
    # Activity 2
    dp = create_datapackage(
        fs=ZipFileSystem(fixture_dir / "mc_complete.zip", mode="w"),
    )
